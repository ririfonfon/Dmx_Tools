void construct_arnet_packet()
{
  ...
  ArtdmxBuffer[8] = ARTNET_ARTDMX;
  ArtdmxBuffer[9] = ARTNET_ARTDMX >> 8;
  //protocole
  ArtdmxBuffer[10] = 0;
  ArtdmxBuffer[11] = 14;
  //sequence
  ...
  //data length
  ArtdmxBuffer[16] = 512 >> 8;
  ArtdmxBuffer[17] = 512;

  for (int t = 0; t < 512; t++)
  {
    ArtdmxBuffer[t + 17 + 1] = buffer_dmx[t];
  }
...
void artDMXReceived(unsigned char* pbuff) {
  ...
  if ( numberOfDimmers > 0 ) {
    if ( uni == 'A') {
      ...
      ESP8266DMX.setSlot(int(&pbuff[ARTNET_ADDRESS_OFFSET]), numberOfDimmers);
